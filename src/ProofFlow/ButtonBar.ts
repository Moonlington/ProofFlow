...
import { cmdInsertCode, cmdInsertMarkdown, cmdInsertMath } from "./commands/commands";
...
export class ButtonBar {
    ...
    render(parentElement: HTMLElement) {
        ...
        const buttonCommands = [
            { name: 'Markdown', command: cmdInsertMarkdown(this._schema, InsertionPlace.Underneath) },
            { name: 'Code', command: cmdInsertCode(this._schema, InsertionPlace.Underneath) },
            { name: 'Math', command: cmdInsertMath(this._schema, InsertionPlace.Underneath) },
    ...
    private getCurrentCell(): Node {
        ...
        for (let depth = $from.depth; depth > 0; depth--) {
            ...
            if (node.type.name === 'codecell' || node.type.name === 'markdown' || node.type.name === 'math_display') {
                return node;
            }
        }
