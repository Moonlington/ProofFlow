...
class Area {
    ...
    constructor(type: Exclude<AreaType, AreaType.Collapsible|AreaType.Input>, content: string) {
        this.id = nextAreaId++;
        this.type = type;
...
class CollapsibleArea extends Area {
    ...
    addArea(area: Area): boolean {
        if ([AreaType.Collapsible, AreaType.Input].includes(area.type)) return false;
...
class ProofFlowDocument {
    ...
    addAreaAfter(area: Area, afterId:number): boolean {
        ...
        if (afterArea.parent !== undefined) {
            afterArea.parent.subAreas.splice(afterArea.parent.subAreas.indexOf(afterArea), 0, area);
            return true; 
        }
    ...
    replaceArea(newArea: Area, replaceId:number): boolean {
        ...
        if (replacedArea.parent !== undefined) {
            replacedArea.parent.subAreas.splice(replacedArea.parent.subAreas.indexOf(replacedArea), 1, newArea);
            return true; 
        }
