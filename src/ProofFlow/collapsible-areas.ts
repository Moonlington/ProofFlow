    ...
    state: {
      ...
      apply(
        ...
        return tr.docChanged
          ? getCollapsibleDecorations(newState, collapsibleNodeType)
          : value;
      },
    },
    props: {
...
function getCollapsibleDecorations(
  state: EditorState,
  collapsibleNodeType: NodeType,
): DecorationSet {
  const collapsibles = findChildrenWithType(
    state.doc,
    collapsibleNodeType,
...
function createCollapseDOM(
  view: EditorView,
  collapsible: { node: Node; pos: number },
) {
  ...
  collapsibleElement.addEventListener("click", (ev: MouseEvent) => {
    const state = view.state.doc.nodeAt(collapsible.pos)?.attrs
      .visible as boolean;
